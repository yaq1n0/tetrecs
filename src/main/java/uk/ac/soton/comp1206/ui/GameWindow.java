...
public class GameWindow {
     ...
     */
    public void startScores(AbstractGame game, boolean checkHighScore) {
        loadScene(new ScoresScene(this, game, checkHighScore));
    }

    /**
     * Display the new high score scene
     */
    public void startNewHighScoreScene(AbstractGame game, ArrayList<Pair<String, Integer>> localScores, ArrayList<Pair<String, Integer>> onlineScores, int caseNumber) {
        loadScene(new NewHighScoreScene(this, game, localScores, onlineScores, caseNumber));
    }
    ...
    public void loadScene(BaseScene newScene) {
        ...
        // binding all new scenes to go to start menu using startMenu(loadScene) when escape is pressed
        scene.setOnKeyPressed(keyEvent -> {
            if (keyEvent.getCode().equals(KeyCode.ESCAPE)) {
                if (currentScene instanceof ChallengeScene) {
                    ((ChallengeScene) currentScene).gameOver();
                } else if (currentScene instanceof MultiplayerScene) {
                    ((MultiplayerScene) currentScene).gameOver();
                    getCommunicator().send("PART");
                } else {
                    startMenu();
                }
            }
